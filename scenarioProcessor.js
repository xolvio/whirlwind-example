...
const influx = new Influx.InfluxDB({ // here we create an InfluxDB client, it works great with Graphana visualisation tool
...
sendPointsToInflux = () => { // here we send our data to DB
...
const throttledSendPointsToInflux = throttle(sendPointsToInflux, sendDataToDbEveryMs, {
  trailing: true,
  leading: false
...
module.exports.generateNewUserEmailInFunction = (context, ee, next) => { // here we use faker to generate random user data
...
module.exports.logResponseToConsole = (requestParams, response, context, ee, next) => {
